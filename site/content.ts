...
export function renderPilot(report: CalibrationReport): string {
  ...
  const downloads = report.sources.map((source) => `<li><a href="/benchmark/pilot-v0/${escapeHtml(source.file)}" download>${escapeHtml(source.file)}</a></li>`).join("\n");
  return `<div class="table-scroll" role="region" aria-label="Pilot comparison, scroll horizontally for all columns" tabindex="0"><table class="ladder-table results-table"><caption>Screened legacy pilot: 190 instances per model, minus provider errors. One response per instance. No tools.</caption><thead><tr><th scope="col">Requested model</th><th scope="col">Strict</th><th scope="col">Final block</th><th scope="col">Strict 95% interval</th><th scope="col">Pass / valid</th><th scope="col">Errors</th></tr></thead><tbody>${rows}</tbody></table></div>
<details class="report-details"><summary>Show strict pass rates by tier</summary><div class="table-scroll" role="region" aria-label="Per-tier pilot results" tabindex="0"><table class="ladder-table results-table"><caption>Same screened subset. Tier 6 has no retained legacy cells and is not scored here.</caption><thead><tr><th scope="col">Requested model</th>${tiers.map((tier) => `<th scope="col">t${tier}</th>`).join("")}</tr></thead><tbody>${tierRows}</tbody></table></div></details>
<details class="report-details"><summary>Download all 3,000 attempt records</summary><p>Gzipped JSONL, one file per model. Synthetic prompts and replies are retained; private upstream error bodies are redacted. The original recorded verdict is kept for audit; the report recomputes scoring.</p><ul class="doc-list dataset-links">${downloads}</ul></details>`;
}
