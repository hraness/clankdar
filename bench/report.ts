...
import { cellSupported, poolForVersion, KNOWN_SUITE_VERSIONS, FAMILIES, FRONTIER_FAMILIES, AGENT_FAMILIES, ALGAL_FAMILIES } from "../ladder/mod.ts";
...
export function buildReport(runs: RecordedRun[], excludedFamilies: string[] = []) {
  ...
  if (excludedFamilies.some((family) => ![...FAMILIES, ...FRONTIER_FAMILIES, ...AGENT_FAMILIES, ...ALGAL_FAMILIES].some((f) => f.name === family))) throw new Error("unknown excluded family");
