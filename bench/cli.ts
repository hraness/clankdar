...
import { adapterByName, openai, openaiAgent, oracle } from "./adapters.ts";
...
const HELP = `usage: bun bench --adapter <oracle|echo|openai:model|agent:openai:model> [options]
...
export async function main(args = process.argv.slice(2)): Promise<number> {
  ...
  if (values["dry-run"] || ((remote || remoteAgent) && !values.execute)) {
    console.log(JSON.stringify({ ...manifest, dryRun: true, maximumRequests: manifest.instances * (remoteAgent ? 30 : 3) }, null, 2));
    return 0;
  }
  if ((remote || remoteAgent) && (!budget || integer(values["max-requests"]!, 30_000) < manifest.instances)) throw new Error("--execute requires --max-requests at least equal to the instance count");
