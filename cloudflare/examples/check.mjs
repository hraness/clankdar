 ...
 * state in memory; supply an awaited checkpoint(record) to privately persist
 * the ticket and responses before submission. After a network/publication error,
 * retry that same check's responses endpoint; never issue a replacement check.
